<template>
	<popup-bottom :show="show" @close="closeSkuBox">
		<view class="sku-box">
				<view class="sku-header container">
					...
					<view class="sku-goods-info">
						<view class="money" :style="{color:themeColor}">
							<text class="symbol fs-26">积分</text>
							<text class="amount fs-38">{{selectSkuInfo[cbPrice]}}</text>
						</view>
				...
				<view class="confirm-btn container" :class="{disabled:selectSkuInfo[cbStock]==0}" :style="{backgroundColor:themeColor}" @click="handleConfirm"><!-- {{selectSkuInfo[cbStock]==0?'立即购买':'缺货中'}} -->立即兑换</view>
			</view>
	</popup-bottom>
</template>
